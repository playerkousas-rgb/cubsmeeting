...
(function () {
  ...
  /* 官方內容以《幼童軍訓練綱要》2026 年第十版第三章「活動徽章及其他徽章」為準，
     ...
     ⚠ 以下 4 個係舊制章，2026 第十版目錄已經冇佢哋，所以刻意唔放入 badgeGroups：
       勞作章（舊制頁45，內容由 2026 手藝章承接）、讀圖章（舊制頁50）、
       電腦章（舊制頁37，內容由 2026 資訊科技章承接）、體操章（舊制頁44，由 2026 體適能章取代）
  ...
  App.openBadge = function(name){var official='https://prog.scouting.org.hk/cub/training-scheme/',
      ...
      spec=App.officialBadges[name] || App.officialBadges[String(name).replace(/（[^）]*）/g,'').trim()], officialHtml=spec?'<p><b>目的：</b>'+esc(spec.purpose)+'</p>'+(spec.rule?'<p class="mut">'+esc(spec.rule)+'</p>':'')+'<ol>'+spec.items.map(function(x){return '<li>'+esc(x)+'</li>';}).join('')+'</ol>'+(spec.note?'<p class="safe">'+esc(spec.note)+'</p>':'')+'<p class="mut">官方綱要頁 '+spec.page+'</p>':'<p>官方內容正在按 2026 年第十版逐章內置。</p>';Modal.open('<h2>🎖️ '+esc(name)+'</h2><div class="subtabs badge-detail-tabs"><button class="subtab cur" onclick="App.switchBadgeTab(this,\'official\')">官方要求</button><button class="subtab" onclick="App.switchBadgeTab(this,\'suggest\')">建議考核</button></div><div id="badge-official">'+officialHtml+'<p class="mut">官方內容來源：《幼童軍訓練綱要》2026 年第十版第三章（活動徽章及其他徽章）。</p></div><div id="badge-suggest" class="hidden"><p>建議領袖流程：</p><ol><li>先逐項讀清楚要求及安全限制。</li><li>先示範，再讓成員按要求完成。</li><li>按官方要求即場判斷是否達標；不達標便安排再試。</li></ol><p class="mut">這一頁只是使用方法，不取代官方要求。</p></div><div class="attachment-row"><a class="btn ghost" href="'+official+'" target="_blank" rel="noopener">📎 開官方訓練綱要附件</a></div><button class="btn" onclick="Modal.close()">關閉</button>');};
