...
(function () {
  ...
  App.vLibrary = function (skills) {
    ...
    /* 活動同技能帶領卡係同一頁（用戶要求放埋一齊），skills 參數保留但唔再分流 */
    var title = '🎮 活動・技能帶領卡';
    var jungleCard = '<section class="card"><h2>🌳 想加一段故事？</h2><a class="btn" href="#jungle">森林故事・角色卡 →</a></section>';
    return jungleCard+'<section class="card"><a class="back" href="#plan" style="text-decoration:none;color:var(--ord)">‹ 返回</a><h2>'+title+'</h2><p class="mut">跟 ghmeeting 同款活動架：每個活動淨出名＋分鐘＋「▶ 即開」，詳細收埋喺卡入面。</p><div class="manual-grid" style="grid-template-columns:repeat(auto-fill,minmax(160px,1fr))">'+cards.join('')+'</div></section>';
  };
  ...
  App.badgeAssess = {
    /* ---- 頁27–28 童軍技能（綠） ---- */
    '露營章':{how:['開營時叫佢跟你落釘、拉繩、起營幕；起完叫佢自己再檢查一次。','膳食由佢落手：洗菜、睇火、派飯、執枱。你企喺旁邊睇安全。','拆營時叫佢数返清楚所有釘同繩。'],
      pass:'八項全部有佢親手做过，唔係企喺度睇你做。營幕起得起、拆得返，膳食有份落手，就算達標。生火同用刀一定要有領袖同行。'},
    ...
    '共融章':{how:['先傾：乜嘢叫多元共融？用團員自己嘅例子講。','介紹特能童軍活動係乜。','身體／教育／文化／種族四類揀兩類，每類講佢哋嘅特質同需要。','安排一次聯合活動或者服務，做完叫佢寫低三句感受。'],
      pass:'四項齐：講得出多元共融、認識特能童軍、講到兩類特殊需要人士嘅特質、真係做过一次聯合活動或服務並有簡單記錄。'},
    ...
    '香港歷史章':{how:['先傾佢想做邊個主題（歷史名人／古舊建築／歷史大事／其他），寫低。','剪貼簿或者網站自己喺屋企做，集會只係跟進度。','揀三個法定古蹟，每個畫一幅海報介紹背景同參觀方法。','參觀博物館之前要攞團長同意；返嚟叫佢當面匯報。','叫佢講一條街或者一個地區名嘅命名典故。'],
      pass:'四項齐：作品完成、三幅古蹟海報、参观過一間合資格博物館並已匯報、講得出街道或地區命名典故。'},
    '國家安全大使章':{how:['官方要求唔喺綱要入面，喺「積極公民」獎章網站。開網站同成員一齊睇佢要考邊個章。', '跟網站列嘅項目逐項做，每項留低證明（相片、證書或活動紀錄）。', '做完由幼童軍團長或班領導人核對，再向青少年活動署申請。'],
      pass:'跟「積極公民」獎章網站列明嘅該章要求完成並取得證書。本套包唔另設標準。'},
    /* ---- 頁38–47 多元創藝（藍） ---- */
    '藝術章':{how:['先同佢傾做邊三項，寫低喺卡上。','每項做完即場影相存檔，唔好留到最後先追。','每項完成叫佢用一句話講：我做咗乜、用咗乜方法。','三項齐晒，砌個小展示俾其他團員睇。'],
    ...
    '娛樂章':{how:['先傾做邊三項，寫低。','默劇、木偶戲、雜技、樂器、舞蹈、土風舞、唱歌、節奏樂器、講故事——每項排練最少一次先正式表演。','每項表演要真係有觀眾（團員就得），唔好淨係排練當完成。'],
      pass:'完成三項唔同項目，每項都有實際表演過。講故事要够五分鐘；雜技或魔術要三个以上連串；揀「其他」要先得到你同意。'},
    ...
    '媒體製作章':{how:['先傾做邊兩項，寫低。','海報／宣傳單張／橫額、簡易網站、電子書、三分鐘音頻故事、三分鐘主題短片、一分鐘動畫——每項做完即場睇一次。','網站要經你同意先可以發佈。'],
      pass:'完成兩項唔同項目，成品睇得到。音頻故事要有音樂同聲效；短片要有對白或旁白；動畫要够一分鐘。'},
    ...
    '寫作章':{how:['先確認佢而家考邊個進度章，因為指定字數唔同。','傾做邊三項，寫低。','詩要至少六行；完成後同佢傾詩句構造同意義。','交返嚟嘅作品要数字數，唔够就叫佢補。'],
      pass:'完成三項唔同項目，每項字數够：幼童軍歷奇章不少於 50 字、高級歷奇章不少於 150 字、金紫荊獎章不少於 250 字。'},
    /* ---- 頁48–52 科學與大自然（青） ---- */
    '天象章':{how:['先用星圖或 App 認黃道十二星座，逐個講主要特徵。','用燈同波示範月相、日食、月食。','太陽系行星逐個講名同特徵。','傾觀星禮儀同光污染對觀星嘅影響。','一齊做針孔投影儀觀太陽。','最後一項二選一：晚間戶外觀星，或者參觀香港太空館。'],
      pass:'六項全做到。觀星要真係去过一次，或者真係去过太空館。觀太陽必須用針孔投影，千祈唔好直接用眼或望遠鏡望太陽。'},
    ...
    /* ---- 頁53–67 運動與愛好（橙） ---- */
    '射箭章':{how:['呢章要合資格教練指導，唔好自己開弓。','知識部分教一次就得，考高一级時無須重考。','訓練四項每次都要重新做：安全規則、基本姿勢、記錄積分、不少於 8 小時訓練。','實射按級別計環數，由教練記錄。'],
      pass:'知識、訓練、實射三部分全齊。實射環數要够該級標準，例：初級 10 米 122 厘米靶連續 6 箭，反曲弓不少於 24 環。'},
    '田徑章':{how:['四個項目全部要做：跳遠、跳高、擲豆袋、50米急跑。','每項試三次，取最好成績。','按官方評分表計分，分 10 分、7 分、5 分三檔。','加總四項分數，對返該級門檻。'],
      pass:'四項全部有成績，總分够該級：初級 22 分、中級 28 分、高級 34 分。少做一个項目就唔算完成。'},
    ...
    '運動章':{how:['先叫佢揀兩種運動，逐種講玩法、基本規則、需要乜裝備、有乜安全事項。','由兩種入面揀一種深入，之後要真係恆常参加。','傾該項運動嘅用具同服裝點保養。','喺團遊戲入面觀察佢嘅體育精神。','最後傾：運動後點解要清潔同更衣，點保護身體各部位（例如足部）。'],
      pass:'五項全做到。第 2 項要真係恆常参加，唔係做过一次就算。體育精神要喺實際遊戲入面睇到。'},
